...
def init_db() -> None:
    with connect() as conn:
        ...
        # Additive: the pre-rendered push body (led by yesterday's feeling) is
        # stored at generation time so the delivery cron can send it later at
        # the user's chosen local time without re-gathering context.
        cursor.execute(
            "ALTER TABLE morning_brief_log ADD COLUMN IF NOT EXISTS push_body TEXT"
        )

        # Per-user morning-notification preferences. The brief is GENERATED by
        # the nightly batch, but DELIVERED by a frequent cron at the user's
        # chosen local time (hour:minute in their IANA tz). last_pushed_day is
        # the brief `day` we most recently pushed — the per-brief dedup guard.
        cursor.execute("""

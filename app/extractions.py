...
the same `(user_id, day)` before calling this — keeping write-side idempotency
...
def store_extractions(parsed: JournalParserResponse, day: str, user_id: UUID) -> None:
    uid = str(user_id)
    with connect() as conn:
        ...
        cursor.execute("""
            ...
            (user_id, day, valence, arousal, primary_quadrant,
             cognitive_labels, cognitive_triggers, social_interactions)
            VALUES (%s, %s, %s, %s, %s, %s::jsonb, %s::jsonb, %s::jsonb)
        """, (
            uid, day, e.valence, e.arousal, e.primary_quadrant,
        ...
        if is_health_meaningful(parsed.health):
            ...
            cursor.execute("""
                ...
                (user_id, day, sleep_quality, exercise_type, diet_quality,
                 somatic_sensations, physical_performance, supplements)
                VALUES (%s, %s, %s, %s, %s, %s::jsonb, %s, %s::jsonb)
            """, (
                uid, day, h.sleep_quality, h.exercise_type, h.diet_quality,
        ...
        if is_productivity_meaningful(parsed.productivity):
            ...
            cursor.execute("""
                ...
                (user_id, day, deep_work_hours, shallow_work_hours,
                 time_block_adherence, cognitive_load, friction_points)
                VALUES (%s, %s, %s, %s, %s, %s, %s::jsonb)
            """, (
                uid, day, p.deep_work_hours, p.shallow_work_hours,
                p.time_block_adherence, p.cognitive_load, json.dumps(p.friction_points),
            ))

        for ev in parsed.events:
            cursor.execute("""
                INSERT INTO events (user_id, day, title, description, tags, event_type)
                VALUES (%s, %s, %s, %s, %s::jsonb, %s)
            """, (uid, day, ev.title, ev.description, json.dumps(ev.tags), ev.event_type))

        for ev in parsed.events:
            for topic in (ev.topics or []):
                if topic.strip():
                    cursor.execute(
                        "INSERT INTO event_topics (user_id, day, event_title, topic) VALUES (%s, %s, %s, %s)",
                        (uid, day, ev.title, topic.strip()),
                    )
